...
async def generate_meta_video(prompt: str, output_path: str, headless: bool = False):
    ...
    if env_cookies:
        try:
            ...
            print(f"ΓÜá∩╕Å Error: Failed to parse META_COOKIES from .env. Ensure it's a valid JSON array. Error: {e}")
            return False
    else:
        print("ΓÜá∩╕Å Warning: No META_COOKIES found in .env. Attempting generation as a guest (may fail or require manual login).")

    print(f"≡ƒÜÇ Initializing Meta AI Video Automation...")
    print(f"≡ƒÄ¼ Prompt: \"{prompt[:60]}...\"")
    
    async with AsyncStealthySession(headless=headless, user_agent=WINDOWS_UA) as session:
        # 2. Inject Cookies before navigation
        if cookie_list:
            print("≡ƒöæ Injecting authenticated session cookies...")
            await session.context.add_cookies(cookie_list)
            
        async def automation_logic(page):
            # A) Stabilize and handle potential popups
            print("≡ƒîÉ Navigating to Meta AI...")
            ...
            try:
                if await page.get_by_text("Welcome to Meta AI").count() > 0:
                    print("ΓÜá∩╕Å Bypassing age verification popup...")
            ...
            print(f"Γ£ì∩╕Å Sending command: {gen_command[:70]}...")
            ...
            print("ΓÅ│ Waiting for Meta AI to render the video (this can take 1-3 minutes)...")
            try:
                ...
                if not video_src:
                    video_src = await video_el.locator('source').first.get_attribute("source")
                
                if not video_src:
                    raise Exception("Video generated but no source URL found.")

                # D) Binary Download via Browser (Bypasses CDN 403 blocks)
                print("≡ƒôÑ Downloading high-fidelity binary...")
                ...
                print(f"Γ£à Success! Video saved to: {output_path}")
                return True

            except Exception as e:
                print(f"Γ¥î Generation failed: {e}")
