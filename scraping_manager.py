...
async def scrape_niche_clips(headless=False):
    ...
    if not json_files:
        print(f"⚠️ No JSON configs found in {CONFIGS_DIR}. Run --patch first.")
        return

    print(f"🕵️  Scraper Orchestrator: Found {len(json_files)} videos to check.")
    llm = LLMHandler()
    ...
    for jf in sorted(json_files):
        ...
        for i, prompt_core in enumerate(visual_prompts):
            ...
            if os.path.exists(output_file) and os.path.getsize(output_file) > 1000:
                print(f"   ✅ Scene {i+1}: Clip exists. Skipping.")
                continue
            ...
            if not master_prompt:
                print(f"   ⚠️  Architect failed. Using raw fallback.")
            ...
            if success:
                print(f"   🎉 Scene {i+1}: SUCCESS! Clip saved.")
            else:
                print(f"   ❌ Scene {i+1}: FAILED after all negotiation attempts.")
